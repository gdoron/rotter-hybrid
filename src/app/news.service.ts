...
export class NewsService {
    ...
    wrapUrlRegex = /(?:^|\s|\>)((https?:\/\/)[\w-]+(\.[\w-]+)+\.?(:\d+)?(\/[^\s\<]+)?)/gim;
        ...
        return this.http.get(`http://rotter.net/gilad/?forum=${forum}&om=${om}`).toPromise()
            .then(response => {
                ...
                for (var i = 0; i < comments.length; i++) {
                    comments[i].html = comments[i].html.replace(this.wrapImageRegex, '<img src="$1" class="responsive-img" onload="this.className += \' loaded\'"/>');
